...
package :deployer_capistrano_dirs do
  ...
  sites_path = '/home/deployer/apps' # TODO: config?
  
  noop do
    pre :install, "mkdir -p #{sites_path}/releases"
    pre :install, "mkdir -p #{sites_path}/shared"
    pre :install, "mkdir -p #{sites_path}/shared/config"
    pre :install, "mkdir -p #{sites_path}/shared/log"
    pre :install, "mkdir -p #{sites_path}/shared/pids"
    pre :install, "mkdir -p #{sites_path}/shared/system"
    
    pre :install, "chown -R deployer:deployer #{sites_path}/"
    pre :install, "chmod -R ug=rwx #{sites_path}/"
  end

  verify do
    has_directory "#{sites_path}/releases"
    has_directory "#{sites_path}/shared"
    has_directory "#{sites_path}/shared/config"
    has_directory "#{sites_path}/shared/log"
    has_directory "#{sites_path}/shared/pids"
    has_directory "#{sites_path}/shared/system"
  end
end
